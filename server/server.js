...
// ── Shared allowed origins (single source of truth) ──
const ALLOWED_ORIGINS = [
  'http://localhost:5500',
  'http://127.0.0.1:5500',
  'https://rural-care360-pcu.vercel.app'
];

// Socket.io setup — must use same origins as Express CORS
const io = new Server(server, {
  cors: {
    origin: ALLOWED_ORIGINS,
    methods: ['GET', 'POST'],
...
io.on('connection', (socket) => {
  ...
  socket.on('sendMessage', async (data) => {
    ...
    try {
      const consultation = await Consultation.findById(consultationId);
      if (consultation) {
        consultation.messages.push({ sender: senderId, message });
        await consultation.save();
        io.to(consultationId).emit('receiveMessage', {
          senderId,
          message,
  ...
  socket.on('typing', (data) => {
    socket.to(data.consultationId).emit('userTyping', { userId: data.userId });
  });
